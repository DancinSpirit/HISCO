...
const loadState = async function(x){
    ...
    $(`#sub-${previousState}`).html(component);
    $("#button-blocker").remove();
    keyButtons = true;
}

const loadStates = async function(){
    if(!user.firstName&&(states[0]!="login"&&states[0]!="register")){
        states = ["login"];
        databaseObjects = [false,false];
        customData = [false,false];
        window.history.replaceState({states:states,databaseObjects:databaseObjects,customData:customData}, "login", "/login");
    }else if(user.firstName&&(states[0]=="login"||states[0]=="register")){
        states = ["home"];
        databaseObjects = [false,false];
        customData = [false,false];
        window.history.replaceState({states:states,databaseObjects:databaseObjects,customData:customData}, "home", "/home");
    }
