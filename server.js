...
app.get("/", function(req,res){
    res.render('base',{states: ["login"], databaseObjects: [false], customData: [false]});
})
...
app.get("/data/:databaseObject/:id", async function(req,res){
    const data = await db[req.params.databaseObject.charAt(0).toUpperCase() + req.params.databaseObject.slice(1)].findById(req.params.id).populate("files").populate("folders");
    res.send(data);
})
...
app.post("/replace/:databaseObject/:id", async function(req,res){
...
/* Update specific property of object */
...
app.post("/component/:component", async function(req,res){
    ...
    if(req.body.databaseObjects != "false"&&(typeof req.body.databaseObjects != "undefined")){
        for(let x=0; x<req.body.databaseObjects.length; x++){ 
            console.log(req.body.databaseObjects[x])
            if(req.body.databaseObjects[x].name=="Folder"){
                data[req.body.databaseObjects[x].name.toLowerCase()]  = await db[req.body.databaseObjects[x].name].findById(req.body.databaseObjects[x].id).populate("files").populate("folders");
            }else{
    ...
    if(url=="components/home"){
        databaseObjects = data.folder = await db.Folder.findById("631fa130dfbd34482cc0795d").populate("files").populate("folders");
    }
